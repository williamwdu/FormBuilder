...
$sql="SELECT * FROM RailRequest.contact;";
	...
	$pdf->Textcol($row['office']);
	$pdf->h2('');
	$pdf->Leftcol("Distance to Shut-off Valve on Each Side of R/W",80);
	$pdf->Textcol($row['office']);
	$pdf->Leftcol("Types of Seals at Ends of Crossings",80);
	$pdf->Textcol($row['office']);
	$pdf->Leftcol("Cathodic Protection (Type)",80);
	$pdf->Textcol($row['office']);
	$pdf->Leftcol("Casing Filler",80);
	$pdf->Textcol($row['office']);
	$pdf->h2('Longitudinal Pipeline');
	$pdf->Leftcol("Distance from Centerline of Outside Track",80);
	$pdf->Textcol($row['office']);
	$pdf->Leftcol("Depth of Bury to Top of Pipe",80);
	$pdf->Textcol($row['office']);
	$pdf->custompagenumber();
	...
	$pdf->AddPage();
	$pdf->customheader($logo,$title);
	$pdf->h1('Section 5: Overhead Strucutre Infomation');
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'New Construction',1);
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'Reconstruction',1);
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'Communication Line Crossing',1);
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'Power Line Crossing',1);
	$pdf->box(4);
	$pdf->h2('Existing Facility');
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'Communication Line',1);
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'Supply (Electrical) Line',1);
	$pdf->h2("Height Above Top of Rail in (ft)");
	$pdf->Leftcol('Supply');
	$pdf->Textcol($row['office'],50);
	$pdf->Leftcol('Communication');
	$pdf->Textcol($row['office']);
	$pdf->h2("General");
	$pdf->Leftcol('Angle of Crossing with Tracks');
	$pdf->Textcol($row['office'],50);
	$pdf->Leftcol('Length of Span (ft)');
	$pdf->Textcol($row['office']);
	$pdf->Leftcol('Height Above Top of Rail (ft, No Wind, 60 deg)',100);
	$pdf->Textcol($row['office']);
	$pdf->h2("Poles");
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'Existing');
	$pdf->checkbox($pdf->GetX(),$pdf->GetY(),'New',1);
	$pdf->box(2);
	$pdf->Leftcol('Length of Span (ft)');
	$pdf->Textcol($row['office']);
	$pdf->Leftcol('Circumference of Top of Pole (in)');
	$pdf->Textcol($row['office']);
	$pdf->Leftcol('Location of Pole with Respect to R/W (ft)');
	$pdf->Textcol($row['office']);
	$pdf->Leftcol('Pole Inside of R/W (ft) Left');
	$pdf->Textcol($row['office']);
	$pdf->Leftcol('Pole Inside of R/W (ft) Right');
	$pdf->Textcol($row['office']);
	$pdf->Leftcol('Depth of Pole Below Ground surface (ft, Min 5\'');
	$pdf->Textcol($row['office']);
	$pdf->h2("Cable");
	$pdf->Leftcol('Type');
	$pdf->Textcol($row['office'],55);
	$pdf->Leftcol('Number',20);
	$pdf->Textcol($row['office'],10);
	$pdf->Leftcol('Size');
	$pdf->Textcol($row['office']);
	$pdf->h2("Fiber");
	$pdf->Leftcol('Type');
